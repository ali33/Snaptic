...
- **Ngôn ngữ giao tiếp với người dùng: tiếng Việt.** Mọi chuỗi hiển thị trên UI, mọi thông báo lỗi, mọi commit message đều bằng tiếng Việt. Tên định danh trong code bằng tiếng Anh — **trừ tên method test, được đặt bằng tiếng Việt** (tên test là câu mô tả hành vi, không phải định danh API; đọc tiếng Việt dễ hiểu hơn hẳn khi test đỏ). Comment bằng tiếng Việt.
...
- Consumes: (không có)
- Produces:
  - `Snaptic.Core.Images.CaptureResult(SKBitmap image, DateTimeOffset capturedAt)` — sealed CLASS (không phải record: `with` sẽ chia sẻ SKBitmap)
...
**Hai lỗi đã phát hiện khi thi công thật — code ở Step 5 đã bao gồm cách sửa:**

1. **UTF-8.** ZXing mặc định ISO-8859-1, bảng đó không có chữ Việt có dấu →
   "Tiếng Việt" ra "Ti?ng Vi?t". Cả `EncodingOptions.Hints[CHARACTER_SET]` lẫn
   `DecodingOptions.CharacterSet` phải đặt "UTF-8".

2. **Bề rộng phải là bội số nguyên của số module.** Đây là lỗi kế hoạch không
   lường trước. Barcode 1D mã hoá dữ liệu bằng đúng TỈ LỆ bề rộng vạch
   (1:2:3:4 module). Áp bề rộng cố định 400px cho EAN-13 (113 module) ra
   3.54 px/module — biên bị làm tròn lệch, vạch 2-module trông thành 3-module,
   và mã **không đọc lại được** dù nhìn vẫn như mã thật. Đo thực nghiệm:
   400px → NULL; 339/452/565 (bội số của 113) → đọc tốt. Số module thay đổi
   theo nội dung với Code128/Code39 nên không ghim hằng số được — phải hỏi
   ZXing bằng `writer.encode(text, format, 0, 0).Width` rồi nhân số nguyên lần.
...
vì barcode (~10ms) và OCR (~10-30ms) phải cập nhật nút độc lập.
...
> **⚠ TUYỆT ĐỐI KHÔNG tự viết `InitializeComponent()` trong Window.** Trình sinh mã
> của Avalonia đã tạo sẵn `public void InitializeComponent(bool loadXaml = true)` —
> chính hàm ĐÓ mới gán các field `x:Name` (`FrozenImage`, `SelectionRect`...). Tự viết
> một bản không tham số là tạo overload khác; C# sẽ chọn bản của bạn, XAML vẫn nạp
> nhưng **mọi field x:Name đều null** → NullReferenceException trong ctor. Bug này đã
> xảy ra thật khi thi công Task 13. Chỉ gọi `InitializeComponent()` trong ctor, không
> định nghĩa nó. (`Application` thì khác: `App.axaml.cs` override `Initialize()` và gọi
> `AvaloniaXamlLoader.Load(this)` là đúng.)
