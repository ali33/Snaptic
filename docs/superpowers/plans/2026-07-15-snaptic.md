...
- Produces: `PreviewViewModel(SKBitmap image, CaptureAnalysis analysis, RecognitionService, IClipboardService, AppSettings)` với các property bool `CanCopyImage`, `CanSave`, `CanCopyDataUri`, `ShowCopyCode`, `ShowOpenLink`, `ShowCopyOcr`, string `CopyCodeLabel`, `OpenLinkLabel`, và `Task RunRecognitionAsync()`
...
public class PreviewViewModelTests
{
    ...
    [Fact]
    public void Nut_co_ban_luon_bat_ngay_ca_khi_chua_phan_tich_xong()
    {
        var vm = Build(new CaptureAnalysis());   // cả hai đều Pending

        Assert.True(vm.CanCopyImage);
        Assert.True(vm.CanSave);
        Assert.True(vm.CanCopyDataUri);
    }
    ...
    public void Analysis_Empty_khong_hien_nut_theo_ngu_canh_nao()
    {
        ...
        Assert.False(vm.ShowOpenLink);
        Assert.False(vm.ShowCopyOcr);
        Assert.True(vm.CanCopyImage);   // nhưng nút cơ bản vẫn có
    }
...
public sealed class PreviewViewModel : INotifyPropertyChanged
{
    ...
    // ---- Nút cơ bản: luôn có ngay, không chờ phân tích ----
    public bool CanCopyImage => true;
    public bool CanSave => true;
    public bool CanCopyDataUri => true;
...
dotnet test
```
Kỳ vọng: PASS toàn bộ — 113 test (70 Core + 43 App).
...
113 test tự động cho phần logic; phần platform có checklist tay."
git push
```
...
**Số test dự kiến:** Core 70 · App 43 · **tổng 113**.
